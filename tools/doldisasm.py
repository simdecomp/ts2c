...
# Usage: doldisasm.py DOL_FILE [ELF_FILE] > assembly_file.s
#
...
def add_label(addr, name, force = False):
    labels.add(addr)
    if force or (name != None and not addr in labelNames):
...
if len(sys.argv) >= 3:
    ...
    with open(sys.argv[2], 'rb') as f:
        ...
        for i in range(0, elfsymtab.num_symbols()):
            ...
            if symbol_name in symbolNames:
                symbol_name = "%s_%08X" % (sym.name, sym['st_value'])
...
blacklistedInsns = {
    ...
    PPC_INS_XSMADDADP, PPC_INS_XSMSUBADP, PPC_INS_XVCMPGEDP, PPC_INS_XVNMSUBADP,
    PPC_INS_XSCMPODP, PPC_INS_XSCMPUDP, PPC_INS_XXSPLTW, PPC_INS_XSMADDMDP, PPC_INS_XVMSUBMSP,
    PPC_INS_XVTDIVSP, PPC_INS_XVADDDP, PPC_INS_XVMADDADP, PPC_INS_XVCMPEQDP, PPC_INS_XVSUBDP,
    PPC_INS_VPKUHUM, PPC_INS_VMRGHB, PPC_INS_XVMSUBADP, PPC_INS_XVCMPGTDP, PPC_INS_XVMADDMDP,
